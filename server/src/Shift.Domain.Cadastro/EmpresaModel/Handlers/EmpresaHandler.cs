...
namespace Shift.Domain.Cadastro.EmpresaModel.Handlers
{
...

    {
        ...
        public ICommandResult Handle(AtualizarEmpresaCommand command)
        {
            ...
            //Auditoria
            var auditoria = new LogAuditoria("Cadastro", empresa.GetType().Name, EAcao.Atualizar, empresa.GetType().Namespace, JsonConvert.SerializeObject(empresa), UserIdLogado);
        ...
        public ICommandResult Handle(ExcluirEmpresaCommand command)
        {
            ...
            if (registro == null)
                AddNotification("Empresa", "Não foi possível realizar esta operação. O cód. informado não existe");
            ...
            if (relacionamento)
                AddNotification("Empresa", "Não foi possível realizar esta operação. Empresa possui vínculo com outras entidades da APP");



            //Auditoria
            var auditoria = new LogAuditoria("Cadastro", registro.GetType().Name, EAcao.Excluir, registro.GetType().Namespace, JsonConvert.SerializeObject(registro), UserIdLogado);
